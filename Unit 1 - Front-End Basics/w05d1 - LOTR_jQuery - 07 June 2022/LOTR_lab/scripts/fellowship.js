...
const makeMiddleEarth = () => {
  ...
  lands.forEach(land => {
    const article = $(`<article id="${land}">`)
    article.append(`<h1>${land}</h1>`)
    middleEarthSection.append(article)
  })
...
const makeHobbits = () => {
  ...
  for (let i = 0; i < hobbits.length; i++) {
    const item = $('<li class="hobbit"></li>')
    item.text(hobbits[i])
    hobbitsList.append(item)
  }
...
const makeBaddies = () => {
  ...
  baddies.forEach(baddy => {
    const item = $(`<li class="baddy">${baddy}</div>`)
...
const weWantsIt = () => {
  ...
  // 1. Create a div with an id of 'gollum' and the text 'Gollum'. Add it to Mordor
...
$(
  ()=>{
    ...
    // if written in plain js:
    // let item = document.querySelector('#1')
    // item.onclick = makeMiddleEarth
